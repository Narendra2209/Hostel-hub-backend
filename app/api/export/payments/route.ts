/**
 * /api/export/payments - the fee-payment ledger as CSV (default) or JSON.
 * `month` selects the billing month; `from`/`to` select when money arrived.
 */
...
import { exportPayments } from '@/lib/services/export.service';
...
export const GET = defineRoute({ role: 'ADMIN', rateLimitWeight: 5 }, async ({ request, origin }) => {
  ...
  const file = await exportPayments(query);
